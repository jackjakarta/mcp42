...
mcpFortyTwo uses an embedded SQLite database via `better-sqlite3` — no separate service to run. The file is created automatically on first connection at `SQLITE_PATH` (default `./data/music.db`).
